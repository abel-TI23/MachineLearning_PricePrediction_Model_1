...
def add_all_features(df):
    ...
    rs = gain / (loss + 1e-10) # Menambah epsilon untuk mencegah pembagian dengan nol
    df['RSI'] = 100 - (100 / (1 + rs))
    ...
    tp_rolling_mean = df['TP'].rolling(20).mean()
    tp_rolling_std = df['TP'].rolling(20).std()
    df['CCI'] = (df['TP'] - tp_rolling_mean) / (0.015 * tp_rolling_std + 1e-10) # Menambah epsilon
...
with st.sidebar:
    ...
    ticker = st.text_input("Masukkan Ticker (misal: AAPL, BTC-USD, GC=F)", value="AAPL")
...
if ticker:
    try:
        with st.spinner(f"Mengambil dan memproses data untuk {ticker}..."):
            ...
            # Mengganti nilai infinity dengan NaN agar bisa dihapus oleh dropna()
            df.replace([np.inf, -np.inf], np.nan, inplace=True)
            
            df.dropna(inplace=True)
            
            if df.empty:
                st.error(f"❌ Tidak ada data yang tersisa untuk ticker {ticker} setelah pemrosesan. Coba periode yang lebih panjang atau ticker lain.")
                st.stop()

            # Memuat artefak setelah data diproses
            ...
            # 1. Hasilkan prediksi untuk SELURUH data historis
        ...
        st.info(
            "Grafik ini membandingkan harga aktual (biru) dengan prediksi historis model (oranye). "
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df['Close'], name='Harga Aktual', line=dict(color='dodgerblue', width=2)))

        # Trace untuk Garis Prediksi Historis
        fig.add_trace(go.Scatter(x=df.index, y=df['Harga Prediksi Historis'], name='Hasil Prediksi Historis', line=dict(color='orange', width=1.5, dash='dot')))
        ...
        with st.expander("Lihat Data Terbaru (termasuk kolom prediksi)"):
            st.dataframe(df[['Open', 'High', 'Low', 'Close', 'Harga Prediksi Historis', 'Volume']].tail(10).style.format({
                'Open': '${:,.2f}',
                'High': '${:,.2f}',
                'Low': '${:,.2f}',
                'Close': '${:,.2f}',
                'Harga Prediksi Historis': '${:,.2f}',
                'Volume': '{:,.0f}'
            }))
        ...
        st.error(f"Terjadi kesalahan saat memproses ticker '{ticker}'. Mungkin ticker tidak valid atau ada masalah lain.")
